use super::common::{
    register_user_for_testing, PropertyRateLimitConfig, RateLimitConfig, RateLimitResult,
    RateLimitStatus,
};
...
fn test_property_based_rate_limiting() {
    ...
    let result = update::<_, RateLimitResult>(
        ...
        (charlie_principal_id, "upload_video".to_string(), false),
    )
    ...
    match result {
        ...
        RateLimitResult::Err(msg) => assert!(
    ...
    let result = update::<_, RateLimitResult>(
        ...
        charlie_principal_id,
        "increment_request_count",
        (charlie_principal_id, "create_post".to_string(), false),
    )
    ...
    let upload_status = update::<_, Option<RateLimitStatus>>(
        &pocket_ic,
        rate_limits_canister,
        charlie_principal_id,
    ...
    // The is_limited flag shows if we've reached or exceeded the limit (1 >= 1 = true)
    assert!(upload_status.is_limited, "Unregistered user with 1/1 requests should be marked as limited");

    let post_status = update::<_, Option<RateLimitStatus>>(
        &pocket_ic,
        rate_limits_canister,
        charlie_principal_id,
        "get_rate_limit_status",
