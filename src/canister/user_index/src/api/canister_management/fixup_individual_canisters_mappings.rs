...
pub async fn fixup_individual_canisters_mapping() {
    let user_canisters = CANISTER_DATA.with_borrow(|canister_data| {
        canister_data
    ...
    ic_cdk::spawn(run_task_concurrently(
        user_canisters_futures,
        10,
        result_callback,
        || false,
    ));

    let available_canisters =
        CANISTER_DATA.with_borrow(|canister_data| canister_data.available_canisters.clone());

    let available_canisters_futures =
        available_canisters
            .into_iter()
            .map(|available_canister| async move {
                let result =
                    ic_cdk::call::<_, (String,)>(available_canister, "get_version", ()).await;
                if let Err(e) = result {
                    if e.1.contains("IC0536") {
